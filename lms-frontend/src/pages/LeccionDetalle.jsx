...
function LeccionDetalle() {
  ...
  const [practicaEjecutada, setPracticaEjecutada] = useState(false);
  ...
  useEffect(() => {
    ...
    setTimerActivo(true);
    setTiempoError(null);
    setPracticaEjecutada(false); // reiniciar validador de práctica
  }, [leccionSlug]);
  ...
  // Lecciones interactivas/exámen requieren que el alumno ejecute la práctica
  const requierePractica = ['interactivo', 'examen'].includes(leccion?.tipo);
  const tiempoSuficiente = (tiempoActivo >= duracionMinima || completada) &&
                           (!requierePractica || practicaEjecutada || completada);
  ...
  const textoBloqueado = () => {
    ...
    if (requierePractica && !practicaEjecutada) {
      return 'Ejecuta el ejercicio práctico para continuar';
    }
  ...
  return (
    <div className="container mt-4 lesson-page" data-ai-context="true">
      ...
      {esEstudiante && !completada && (
        <div className={`lesson-reading-bar ${tiempoSuficiente ? 'reading-done' : timerActivo ? 'reading-active' : 'reading-paused'}`}>
      ...
      <div className="lesson-main-grid">
        ...
        <aside className="card p-4 lesson-side-card">
          ...
          <p>
            <strong>Tiempo mínimo requerido:</strong><br />
            {Math.ceil(duracionMinima / 60)} min estimados
          </p>
          <p>
      ...
      <section className="card p-4 mt-4">
        ...
        <AreaPractica
          leccion={leccion}
          onEjecutado={() => setPracticaEjecutada(true)}
        />
        {requierePractica && !practicaEjecutada && esEstudiante && !completada && (
        ...
        <div className="lesson-nav mt-4 mb-5">
          {completada ? (
            ...
            <div className="lesson-complete-block">
              ...
              <button
                className={`btn ${tiempoSuficiente ? 'btn-success lesson-btn-ready' : 'btn-secondary lesson-btn-locked'}`}
                onClick={tiempoSuficiente ? marcarCompletada : undefined}
                disabled={!tiempoSuficiente}
              >
                {tiempoSuficiente
